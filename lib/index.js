 ...
 * session id in the target plus an archived source session). Companion tools
 * `list_qq_bindings` and `rebind_qq_channel` expose the QQ channel mapping
 * maintained by `@tencent-connect/dsh-qqbot` (see ./qq-rebind.js).
 *
...
import { rebindQQChannel, listQQBindings, setQQChannelBinding } from "./qq-rebind.js";
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      "When the source session is bound to a QQ channel (@tencent-connect/dsh-qqbot), its channel mapping is rewritten to the continuation session by default (restart the bot process to apply).",
    parameters: {
      migrations: {
        type: "array",
        required: true,
      ...
      rebind_qq_channel: {
        type: "boolean",
        description: "Rewrite @tencent-connect/dsh-qqbot channel mappings from the source session to the continuation session (default true; no-op when the source has no QQ binding). Takes effect after the bot process restarts."
      }
    },
    output: {
      schema: {
            ...
            items: {
              ...
              properties: {
                ...
                qq_channel: {
                  ...
                  additionalProperties: false,
                  properties: {
                    rebound: { type: "boolean", required: true },
                    session_key: { type: "string" },
                    from: { type: "string" },
                    to: { type: "string" },
                    needs_restart: { type: "boolean" },
                    reason: { type: "string" }
                  }
                }
              }
    ...
    async execute(args) {
      ...
      const rebindQQ = args.rebind_qq_channel !== false;
      ...
      for (const migration of args.migrations) {
        ...
        try {
          ...
          if (rebindQQ && entry.continuation_session_id !== void 0) {
            const qq = rebindQQChannel({
              sourceSessionId: migration.session_id,
              continuationSessionId: entry.continuation_session_id
            });
            if (qq.rebound === true || (qq.reason !== "no-qq-binding" && qq.reason !== "qqbot-not-installed")) {
              entry.qq_channel = qq;
            }
          }
  ...
  ctx.tools.register(defineTool({
    name: "list_qq_bindings",
    description:
      "Read the current @tencent-connect/dsh-qqbot channel state: which DSH session each QQ peer is bound to " +
      "(inbound routing overrides), per-peer model preferences, and known outbound peer entries. Read-only.",
    parameters: {},
    output: {
      schema: {
        type: "object",
        additionalProperties: false,
        properties: {
          qqbot_installed: { type: "boolean", required: true },
          bindings: {
            type: "array",
            items: {
              type: "object",
    ...
    async execute() {
      return listQQBindings();
    }
  }));

  ctx.tools.register(defineTool({
    name: "rebind_qq_channel",
    description:
      "Point a QQ channel (peer sessionKey) at a specific DSH session — the manual counterpart of the automatic " +
      "migration rebind, for switching an ongoing chat between sessions. The sessionKey is resolved from an explicit " +
      "key, a source-session hint, the target's own peer entry (cryptographically verified when possible), or the " +
      "single known binding; a copyable peer entry is seeded for the target. Takes effect after the bot process restarts.",
    parameters: {
      session_id: { type: "string", required: true, description: "Target DSH session that should receive the channel's messages." },
      session_key: { type: "string", description: "Explicit channel key 'qqbot:<appId>:<scope>:<peerId>'. Omit to auto-resolve." },
      source_session_id: { type: "string", description: "Currently bound session; hint to locate the key when several peers are bound." }
    },
    output: {
      schema: {
        type: "object",
        ...
        properties: {
          ...
          peers_seeded: { type: "boolean" },
          needs_restart: { type: "boolean" },
          reason: { type: "string" },
          known_bindings: { type: "array", items: { type: "string" } }
        }
      },
      render: (_args, value) => [{ type: "text", text: JSON.stringify(value) }]
    },
    async execute(args) {
      try {
        await ctx.sessionPersistence.inspect(args.session_id);
      } catch {
        throw new HarnessError(`rebind_qq_channel: session '${args.session_id}' does not exist`, "invalid_request");
      }
      return setQQChannelBinding({
